...
public class JwtUtil {
    ...
    public static String createJwt(Integer id, ProfileRole role) {
        ...
        jwtBuilder.setIssuer("ArzonApteka");
    ...
    public static Integer decodeJwtAndId(String jwt) {
        JwtParser jwtParser = Jwts.parser();
    ...
    public static ProfileJwtDTO getProfile(HttpServletRequest request, ProfileRole requiredRole, ProfileRole requiredRole1) {
        ProfileJwtDTO jwtDTO = (ProfileJwtDTO) request.getAttribute("jwtDTO");
        if (jwtDTO == null) {
            throw new UnauthorizedException("Not authorized");
        }

        if (!jwtDTO.getRole().equals(requiredRole) || !jwtDTO.getRole().equals(requiredRole1)) {
            throw new ForbiddenException("Forbidden exp");
        }
